...
class MinMax:
    ...
    def findPlayerPositions(
        ...
        def helper(playerPosition: tuple[int, int]) -> list[tuple[int, int]]:
            ...
            neighbors: set[tuple[int, int]] = {(-1, 0), (1, 0), (0, -1), (0, 1)}
                ...
                for pos in new_positions:
                    for dx, dy in neighbors:
                        ...
                        if (
                            ...
                            and 0 <= neighbor[0] < len(board)
                            and 0 <= neighbor[1] < len(board[0])
                        ):
    ...
    def simulate(self, colors: list[int], player1: bool, origBoard: Board) -> int:
        ...
        positions: tuple[list[tuple[int, int]], list[tuple[int, int]]] = (
            self.findPlayerPositions(board)
        )
    ...
    def generateTree(
    ...
    ) -> tuple[AnyNode | None, int | float, list[list[int]] | None]:
        if depth == maxDepth:
            return (
                None,
                self.simulate(node.colors, maximizing_player, self.gameBoard),
                [node.colors],
            )
        ...
        for move in self.getAvailableMoves(node.colors[-2], node.colors[-1]):
            ...
            child_node: AnyNode = AnyNode(
                name=f"{node.name}{move}", parent=node, colors=newColors
            )

            # Evaluate the child node using Minimax
            _, eval, path = self.generateTree(
                child_node, maxDepth, depth + 1, a, b, not maximizing_player
            )
    ...
    def evaluate_tree(
        self, board: Board, maxDepth: int, isPlayer1
    ) -> tuple[list[list[int]] | None, int | float]:
        root: AnyNode = AnyNode(
            name="Root", colors=[board.board[6][0], board.board[0][7]]
        )
        best_path, best_score, best_path_colors = self.generateTree(
            root, maxDepth, 0, float("-inf"), float("inf"), isPlayer1
        )
