...
class Tree(MovingCameraScene):
    def construct(self):
        ...
        graph3 = Graph(
            ...
            layout_scale=7,
            labels=False,
            layout_config={"vertex_spacing": (1.25, 2)},
            vertex_config={
                "1": {"fill_color": RED},
                "2": {"fill_color": PURPLE},
                "3": {"fill_color": BLUE},
        ...
        self.play(
            ShowPassingFlash(graph3.copy().set_color(YELLOW), run_time=2, time_width=2)
        )
        self.wait(3)
        ...
        max2 = Text("Minimizer").scale(0.5).set_x(-11).set_y(-2)
        min1 = Text("Maximizer").scale(0.5).set_x(-11).set_y(0)
